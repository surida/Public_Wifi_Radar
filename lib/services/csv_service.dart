...
class CsvService {
  Future<List<WifiInfo>> loadWifiData() async {
    try {
      ...
      final ByteData rawBytes = await rootBundle.load('assets/csv/seoul_public_wifi_data.csv');
      ...
      // Skip header (index 0)
      // User requested to limit to 30 items
      final int limit = rows.length > 31 ? 31 : rows.length;
      for (var i = 1; i < limit; i++) {
        if (i == 1) {
            LogService().log('Row 1: ${rows[i]}');
            LogService().log('Row 1 length: ${rows[i].length}');
            if (rows[i].length > 14) {
                LogService().log('Lat Raw: "${rows[i][13]}", Lng Raw: "${rows[i][14]}"');
            }
        }
        try {
      ...
      LogService().log('CSV Loaded: ${wifiList.length} items parsed from seoul_public_wifi_data.csv');
      return wifiList;
    } catch (e) {
